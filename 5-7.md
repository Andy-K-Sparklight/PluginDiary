...
```json
{
  ...
  "display": {
    ...
    "hidden": false, // 是否在完成后才显示
    "background": "minecraft:textures/gui/advancements/backgrounds/stone.png"
      // 如果没有 parent 则需要，否则不填
      // 表示进度的背景图，可选的内容很有限：
...
首先是 `parent` 键，如果你的成就**具有父节点**，就需要填写这一项。如果你的成就是根节点，就不需要。另外，如果该进度是根节点，那么 Minecraft 会自动在成就页面中用它的图标创建一个新的页面。
...
唯一需要说明的是这里用到的多线程。按照 Bukkit 规定，`Bukkit.XXX` 不能在异步方法中被调用，而 `AsyncPlayerChatEvent` 是个明显的异步方法。
