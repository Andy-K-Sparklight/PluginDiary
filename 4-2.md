...
ps.setString(1, "someKey");
ps.setString(2, "someId");
// SQL 中索引从 1 开始！
```

?> **到底怎么回事**？<br/>这种方法可以避免 SQL 注入，并且由于进行了预编译，SQL 的执行更加高效。

### 执行 SQL

在 Java 中，SQL 语句有 4 个执行方法：

```java
void execute() throws SQLException // 仅执行，无返回
void execute(String sql) throws SQLException // 上一个方法的变种，适用一般的 Statement
ResultSet executeQuery() throws SQLException // 查询类语句
int executeUpdate() throws SQLException // 更新类语句
```

第一个和第三个好理解。第一个不关心返回值，第三个返回受影响的行数。

那 `ResultSet` 是什么捏？

`ResultSet` 是 `SELECT` 返回的一个「切片」，也就是一小张表。

调用它的 `first` 方法选择第一行，`next` 滚动到下一行。

`ResultSet` 有一系列的 `getXXX` 方法（和 YAML 很像）。`getXXX` 获得已选行中的指定列上的值。如果 `getXXX` 与那里实际存储的值无法相互转换，就会出错。

当然了，如果单纯执行 `execute`，`ResultSet` 将无法自由地选择行（会出错），需要在 `executeXXXX` 方法中加入额外的参数（`ResultSet.TYPE_SCROLL_SENSITIVE` 和 ` ResultSet.CONCUR_READ_ONLY`）。

这一点我不打算在这里说明，我们在行动调试中（AC-1-3）中将能够更好说明这一点。目前你只需要知道有这个方法就可以了。
