...
Bukkit 中有一个名为 `ShapedRecipe` 类来完成这项任务。这个类看上去可以直接实例化，很好：
...
正如你想到的，这个 `ItemStack` 也可以 `setAmount` 设置合成的或者对它的 `ItemMeta` 进行 `setLore` 用于获得更漂亮的物品等等。相比而言这比 Mod 和数据包（指客户端本地的那个）更灵活。
...
这里的 `setIngredient` 实际上有两个版本：

```java
ShapedRecipe setIngredient(char symbol, Material material)
// 宽松模式
ShapedRecipe setIngredient(char symbol, ItemStack itemstack)
// 严格模式
```
...
而使用 `ItemStack` 的是「严格模式」，需要名称、介绍（Lore）等等**与设置的物品完全一致**才接受，只要有一点点不一样就不认。例如，一个火把改名为「大火球」可能就不能被该模式识别。

这两种方法各有利弊。第一种对玩家更友好，第二种可以让服务器「仅允许使用特定的物品进行合成」，并且可以指定数量。
...
Bukkit 这里还有一个很坑的地方，服务器要关闭时要**调用 `clearRecipes` 清除我们自定义的合成表**，否则**不知道会有什么后果**！这个应该写在 `onDisable` 方法中。
...
shapelessRecipe = shapelessRecipe.addIngredient(4, requiredItem);
// 不是就地修改！切记！
```
...
这里的原料无法使用严格模式（因为没有对应的实现）。

`Material` 只能指定一种原料，那如果很多原料（例如所有的铁质装备）得到同一种结果呢？这就得编写很多很多的 `BlastingRecipe`！
...
这也和熔炉一样，只是萤火不存在「燃料」的概念，因此应当适当延长烧炼时间。
