...
内存中的东西会在关机后丢失，因此用户的设置必须保存在文件中才能留下来。

你一定已经使用过许多不同的插件了，有些插件的配置很简单，有些长篇大论，不过，这些都是配置。

使用插件时，我们只需要考虑如何把配置写好就行了，插件来读取这些配置。但现在我们成为插件开发者了，需要了解怎么完成读取配置这项工作。
...
可以看到，定义时需要指定类型，后面赋值和使用时就不需要了。

一个变量只能定义一次，但可以多次赋值（`final` 除外）。
...
![JDOC](https://www.picbed.cn/images/2021/02/15/image.png)

那么我们现在就来试试吧！首先我们知道，`FileConfiguration` 类是我们需要找的类，那么我们搜索它……（注意大小写！）

![SEARCH](https://www.picbed.cn/images/2021/02/15/image57759c6c4899b09f.png)
...
![CLAZZ1](https://www.picbed.cn/images/2021/02/15/image6ffc874919672a66.png)

![CLAZZ2](https://www.picbed.cn/images/2021/02/15/image2863ce7e221aebec.png)
...
那么「Field Summary」提供的信息就很明显了吧，它提供的是变量名而没有提直接显示类型，用你的鼠标单击变量名就能查看它的类型了。
...
![GETSSET](https://www.picbed.cn/images/2021/02/15/imageac33cbfe0dc82eaa.png)
...
整数在 Java 中的类型是 `Integer` 或者 `int`，可以通过以下方法获得：
