...
这一次调试，笔者将把修复问题的过程原原本本地展现出来。当然，笔者还是希望能够一次运行成功。
...
回到 IDEA 中，在「HarmonyAuth SMART」工件中，移除「mysql-connector-java-8.0.23」，接着在「Available Elements」中找到它，右键，「Extract Into Output Root」。

![REBUILD](https://www.picbed.cn/images/2021/02/10/image575893fd35f4ea97.png)

「Apply」、「OK」，重新构建，再将插件放到 `plugins` 目录下。
...
验证失败是自然的，但我们默认不是没有启动 MySQL 吗？

哦！对了！我们没有处理这个设置，直接初始化了 `DBDataManager`，需要修改一下。
...
查阅了相关资料后，我发现 `createStatement` 参数中应当传入 `ResultSet.TYPE_SCROLL_SENSITIVE` 用于滚动游标和 `ResultSet.CONCUR_READ_ONLY` 表示只读，这些问题都只出现在 `getXXX` 方法中，我们修改它们就好……先关闭服务器。
...
这里只列出了一处，但所有的我们都要设置。包括 `isExist` 和 `getNextRequest` 中：
...
所以这部分代码应该写在 `PlayerLoginEvent` 中。
...
看见了吧？`firstId.toString()` 应该移动到括号外去。而且我还忘了 `msg.`！除此之外，我们还要修改下面一行。改完后：
...
`float` 的精度没有那么高，因此应将 `0.0001` 改为 `0.001`，也就是改回去啦。

## 修复聊天问题

我们将 `getMessage` 的结果用日志输出，结果无任何返回。

笔者后来才意识到，命令和聊天不是同一个频道发送的！
...
我们会发现，玩家的登录是在玩家加入服务器之前的，也就是说，`PlayerLoginEvent` 发生后，玩家的实体才出现，因此我们应该将减速代码写在 `PlayerJoinEvent` 中。既然不是 `float` 的问题，我们就提升一点精度吧。
...
如果玩家没登录就退出，会导致移动速度卡在 `0.00001`，这个数据保存在了存档里，这可不行啊。
...
这样就除掉了 Bug。对了，在测试前一定要删除 `world`，`world_nether` 和 `world_end` 啊。
...
就这么多了。此外，登录时取消已经创建的请求，重新创建请求时自动覆盖也成功了（查数据库可知）。至于钩子能否运行……既然配置空命令也没出错，我们就权且相信它可以正常运行吧（笑）。
...
回顾你编写的「Hello World」，记忆犹新？很好，那一章是「基础之起点」，而我们下一章的内容，叫做「终极之开端」。
