...
什么是简化呢？我们举个例子，有时候我们只需要设置物品的名称，这时候就得：
...
在 IDEA 的「Project Structure」中添加新模块，随意命名。我就叫它「RarityCommons」。

!> **RarityCommons 的许可协议**<br/>RarityCommons 不适用首页的许可，它遵循 [GNU 通用公共许可证（第三版）](https://www.gnu.org/licenses/gpl-3.0.html)，请仔细阅读该许可证。该许可证也包含在其代码仓库中。<br/>![GPLv3](https://www.picbed.cn/images/2021/02/21/gplv3-with-text-136x68.png)

为它添加依赖「spigot-1.16.5」（参见 EX-1-1）。

在 `src` 下创建包，随意命名（只要你能够记住），我就命名为 `rarityeg.commons`。
...
这是 JavaDocs 的标准语法。由于我们的类库最终可能要被别人使用，因此相关的说明要写在其中。
